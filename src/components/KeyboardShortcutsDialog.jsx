    ...
    <div className="flex items-center gap-1">
      {keys.map((key, index) => (
        <span key={index} className="flex items-center gap-1">
          ...
          <kbd className={`px-2 py-1 text-xs font-mono border rounded transition-all ${
            highlight 
      ...
      <DialogContent className="max-w-lg max-h-[80vh]">
          ...
          {shortcuts.map((section) => (
            <div key={section.category} className="space-y-3">
              <div className="flex items-center gap-2 pb-2 border-b border-border/50">
                <div className="p-1.5 bg-muted/30 rounded text-muted-foreground">
                  {section.icon}
                </div>
              ...
              <div className="space-y-2 pl-8">
                {section.items.map((shortcut, index) => (
                  <div
                    key={index}
                    className="group flex items-center justify-between py-2 px-3 rounded-lg bg-muted/20 hover:bg-muted/40 transition-colors"
                  >
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <div className="w-2 h-2 rounded-full bg-primary/20 group-hover:bg-primary/40 transition-colors flex-shrink-0" />
        ...
        <div className="flex items-center justify-between pt-4 border-t border-border/50 text-xs text-muted-foreground">
          <div className="flex items-center gap-2">
            ...
            <span>Press <kbd className="px-1.5 py-0.5 bg-muted border border-muted/30 rounded text-xs font-mono">Ctrl+H</kbd> to close</span>
          </div>
          <div className="flex items-center gap-1">
